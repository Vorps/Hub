...
public class MenuHelmetHead extends MenuRecursive {
    ...
    private MenuHelmetHead(UUID uuid, ArrayList<ItemBuilder> list) {
        super(uuid, new byte[]{10, 2, 6}, Bukkit.createInventory(null, 54, "§6Boutique > Costumes > Têtes"), new int[][]{{0, 0}, {1, 1}, {2, 2}, {3, 0}, {5, 0}, {6, 2}, {7, 1}, {8, 0}, {9, 1}, {17, 1}, {18, 2}, {26, 2}, {27, 2}, {35, 2}, {36, 1}, {44, 1}, {46, 1}, {47, 2}, {48, 0}, {49, 1}, {50, 0}, {51, 2}, {52, 1}}, list, PlayerData.getLang(uuid), 7, 9, new int[]{12, 13, 14}, Type.DYNAMIQUE, Hub.getInstance());
    }
    ...
    public void initMenu(UUID uuid, int page){
        ...
        if(menu.getItem(52).getType().getId() == 160){
            super.setItem(52, new ItemBuilder(Material.MAP).withName("§6<- Page Précedente -> Chapeaux").withLore(new String[]{"§7Retour au menu Chapeaux"}).get());
        }
        if(menu.getItem(53) == null){
            super.setItem(53, new ItemBuilder(Material.PAPER).withName("§6Page Suivante -> Block").withLore(new String[]{"§7Page Block"}).get());
        }
    }

    public static void createMenu(UUID uuid){
        ArrayList<ItemBuilder> list = new ArrayList<>();
        Products.getProduct(10).forEach((Products product) -> list.add(product.getItem(PlayerData.getLang(uuid)).withLore(new Purchase(uuid, "ce chapeau sur votre tête.").purchase(product.getName()))));
