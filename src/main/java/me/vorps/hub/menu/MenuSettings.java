...
public class MenuSettings extends Menu{
	...
	public MenuSettings(Player player, boolean state){
		...
		if(player.hasPermission("fortycube.build.me.on") || player.hasPermission("fortycube.build.me.off")){
			if(playerData.isBuild()){
				menu.setItem(19, new Item(351).withData((byte) 10).withName("§6Mode build dans le hub").withLore(new String[] {"", "§7Mode build dans le hub", "", "§aCommande §a> §b/build"}).get());
			} else {
				menu.setItem(19, new Item(351).withData((byte) 8).withName("§6Mode build dans le hub").withLore(new String[] {"", "§7Mode build dans le hub", "", "§aCommande §a> §b/build"}).get());
			}
		}
		if(player.hasPermission("fortycube.fly.me.on") || player.hasPermission("fortycube.fly.me.off")){
			if(playerData.isFly()){
				menu.setItem(12, new Item(351).withData((byte) 10).withName("§6Mode fly dans le hub").withLore(new String[] {"", "§7Mode fly dans le hub", "", "§aCommande §a> §b/fly"}).get());
			} else {
				menu.setItem(12, new Item(351).withData((byte) 8).withName("§6Mode fly dans le hub").withLore(new String[] {"", "§7Mode fly dans le hub", "", "§aCommande §a> §b/fly"}).get());
			}
		}
		if(player.hasPermission("fortycube.double_jump.me.on") || player.hasPermission("fortycube.double_jump.me.off")){
			if(playerData.isDoubleJumps()){
				menu.setItem(21, new Item(351).withData((byte) 10).withName("§6Mode jump dans le hub").withLore(new String[] {"", "§7Mode jump dans le hub", "", "§aCommande §a> §b/jump"}).get());
			} else {
				menu.setItem(21, new Item(351).withData((byte) 8).withName("§6Mode jump dans le hub").withLore(new String[] {"", "§7Mode jump dans le hub", "", "§aCommande §a> §b/jump"}).get());
			}
		}
