...
public class MenuGrades extends MenuRecursive {
	...
	private MenuGrades(UUID uuid, ArrayList<ItemBuilder> list){
        super(uuid, new byte[] {11, 4, 5}, Bukkit.createInventory(null, 45, "§6Boutique > Grades") , new int[][] {{0, 0}, {1, 1}, {2, 2}, {3, 1}, {5, 1}, {6, 2}, {7, 1}, {8, 0}, {9, 1}, {17, 1}, {18, 2}, {26, 2}, {27, 1}, {35, 1}, {37, 1}, {38, 2}, {39, 1}, {40, 0}, {41, 1}, {42, 2}, {43, 1}, {44, 0}}, list, PlayerData.getLang(uuid), 7, 9, Type.DYNAMIQUE, Hub.getInstance());
	}

    @Override
    public void initMenu(UUID uuid, int page){
        super.setItem(4, new ItemBuilder(Material.ENCHANTING_TABLE).withName("§6Grades").get());
    ...
    public static void createMenu(UUID uuid){
        ...
        Products.getProduct(6).forEach((Products product) -> list.add(product.getItem(PlayerData.getLang(uuid)).withLore(new Purchase(uuid, "ce grade").purchaseGrade(product.getName()))));
